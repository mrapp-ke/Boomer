...
def __generate_dataset(num_examples: int, num_labels: int, tau: float, p: float, marginal_independence: bool,
    ...
    if marginal_independence:
        ...
        for k in range(num_labels):
            current_x, current_y, current_a = __generate_features_and_labels(num_examples=num_examples, num_labels=1,
                                                                             tau=1, p=p, dependent_error=False,
                                                                             one_error=False, random_state=random_state)
            x[:, (k * 2, k * 2 + 1)] = current_x[:, :]
            y[:, k] = current_y[:, 0]
            a[k, :] = current_a
            random_state += 1
    else:
        x, y, _ = __generate_features_and_labels(num_examples=num_examples, num_labels=num_labels, tau=tau, p=p,
...
def __generate_features_and_labels(num_examples: int, num_labels: int, tau: float, p: float, dependent_error: bool,
    ...
    # It is very important that this random number generator is used everywhere. If you want consistency, that means,
    # that a dataset with num_examples=n is a subset (actually the beginning) of a dataset with num_examples > n, then
    # you have to set the random states of the Bernoulli distribution to something different, but never never to the
    # same seed than here
    np.random.seed(random_state)
    ...
    if one_error:
        ...
        errors = np.tile(bernoulli.rvs(p, size=num_examples, random_state=None).reshape(num_examples, 1), num_labels)
    else:
        # Use explicitly None as random_state, which uses the singleton of np.random, i.e., it re-uses the number
        # generator which we initialized before with np.random.seed(random_state)
    ...
    for r in range(num_examples):
        for c in range(num_labels):
            ...
            # Error is either 0 or 1. If it is 0, nothing changes. If it is 1, then switch the label, which is done by
            # adding -1 if it was 1, or adding 1 if it was 0
            error = errors[r, c] * (-1 if label_set else 1)
            y[r, c] = label_value + error

    if one_error:
        # Flick one random label per example, according to Dembczyński et al. 2012, Sec. 7.5, this sets the Bayes error
        # for Hamming to 1 / m, m = number of labels (1 / m of label-example-combinations are wrong), and to 1 - 1 / m
        # for subset accuracy (because a label combination is changed in m different ways, so the bayes classifier,
        # even if learns the correct pattern on the instance space, it will make the correct prediction for 1 / m
        # examples with that label combinations, and hence (1 - 1 / m) the wrong prediction)
