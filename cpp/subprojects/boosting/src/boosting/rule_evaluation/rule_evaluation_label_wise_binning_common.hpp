...
namespace boosting {
    ...
    static inline float64 calculateLabelWisePredictionInternally(uint32 numPredictions, ScoreIterator scoreIterator,
        ...
        for (uint32 c = 0; c < numPredictions; c++) {
            ...
            uint32 weight = weightIterator[c];

            // Calculate the score to be predicted for the current label...
            float64 score = sumOfHessians + (weight * l2RegularizationWeight);
            ...
            qualityScoreIterator[c] = score + (0.5 * weight * l2RegularizationWeight * scorePow);
            overallQualityScore += score;
        }

        // Add the L2 regularization term to the overall quality score...
        overallQualityScore += 0.5 * l2RegularizationWeight * l2NormPow<ScoreIterator, WeightIterator>(scoreIterator,
                                                                                                       weightIterator,
                                                                                                       numPredictions);
